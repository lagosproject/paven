...
## 2. Ingestion in VVC Reference Software (VTM 18.2)
...
A complete, production-verified patch and 1-click compilation script are provided in this repository at [`tools/codec_runners/vtm_paven/`](file:///media/vant/Elements/PAVEN_RELEASE/github_paven/tools/codec_runners/vtm_paven/):
- **Patch file:** [`tools/codec_runners/vtm_paven/vtm_18_2_paven.patch`](file:///media/vant/Elements/PAVEN_RELEASE/github_paven/tools/codec_runners/vtm_paven/vtm_18_2_paven.patch) (399 lines across 9 files).
- **1-Click Build Script:** [`tools/codec_runners/vtm_paven/build_vtm_paven.sh`](file:///media/vant/Elements/PAVEN_RELEASE/github_paven/tools/codec_runners/vtm_paven/build_vtm_paven.sh).

```bash
# Clone official JVET VTM-18.2 and build with the PAVEN patch in one step:
./tools/codec_runners/vtm_paven/build_vtm_paven.sh /path/to/build_directory
```

### Required Encoder Configuration
The patch adds the following CLI option to `EncAppCfg`:
- `--QPFilePath`: Path to the external `.qp` configuration matrix generated by PAVEN.
