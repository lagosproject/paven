...
class YUVTransformLayer(nn.Module):
    def __init__(self):
        super(YUVTransformLayer, self).__init__()
		
        convType = 2 # 1 for kernel 3, 2 for kernel 1
        if convType == 1:
            self.conv_y = nn.Conv2d(1, 4, kernel_size=3, stride=1, padding=1, bias=False)
            self.conv_u = nn.Conv2d(1, 2, kernel_size=3, stride=1, padding=1, bias=False)
            self.conv_v = nn.Conv2d(1, 2, kernel_size=3, stride=1, padding=1, bias=False)
        elif convType == 2:
            self.conv_y = nn.Conv2d(1, 4, kernel_size=1, stride=1, padding=0, bias=False)
            self.conv_u = nn.Conv2d(1, 2, kernel_size=1, stride=1, padding=0, bias=False)
            self.conv_v = nn.Conv2d(1, 2, kernel_size=1, stride=1, padding=0, bias=False)

        self.conv_final = nn.Conv2d(8, 3, kernel_size=1, stride=1, padding=0, bias=False)

        # Initialize weights so that the layer performs averaging
        self.conv_y.weight.data.fill_(0.25)
        self.conv_u.weight.data.fill_(0.5)
        self.conv_v.weight.data.fill_(0.5)
        self.conv_final.weight.data.fill_(1.0)
    ...
    def forward(self, x):
        ...
        # Reshape input tensor to (batch_size * time_steps, channels, height, width)
        x_reshaped = x.reshape(batch_size * time_steps, channels, height, width)

        #print(x_reshaped.shape)

        # Extract Y, U, and V channels
        y = x_reshaped[:, :1, :, :]  # Extract Y channel
        u = x_reshaped[:, 1:2, :, :]  # Extract U channel
        v = x_reshaped[:, 2:, :, :]  # Extract V channel
        ...
        # Reshape back to (batch_size, time_steps, channels, height, width)
        # yuv = yuv.view(batch_size, time_steps, -1, height, width)
        

        #print(yuv.shape)

        # Apply final convolutional layer
        yuv = self.conv_final(yuv)

        #print(yuv.shape)
	
        yuv = yuv.reshape(batch_size, -1, time_steps, height, width)

        #print(yuv.shape)

        return yuv
	
class YUVVideoSaliencyModel(nn.Module):
    def __init__(self, 
                transformer_in_channel=32,
                nhead=4,
                use_upsample=True,
        ...
        super(YUVVideoSaliencyModel, self).__init__()
        self.yuv_transform = YUVTransformLayer()
        self.video_saliency_model = VideoSaliencyModel(
            transformer_in_channel=transformer_in_channel,
            nhead=nhead,
            use_upsample=use_upsample,
            num_hier=num_hier,
        ...
        self.backbone = self.video_saliency_model.backbone

    def forward(self, x):
        x = self.yuv_transform(x)  # Transform YUV image
        return self.video_saliency_model(x)


class PavenModel(nn.Module):
    """
    High-level PAVEN Video Saliency Model Interface.
    ...
    Provides seamless loading from Hugging Face Hub or local weights files.
    """

    def __init__(
        self,
        transformer_in_channel: int = 32,
        nhead: int = 4,
        use_upsample: bool = True,
        num_hier: int = 3,
        num_clips: int = 32,
    ):
        super(PavenModel, self).__init__()
        self.net = VideoSaliencyModel(
            transformer_in_channel=transformer_in_channel,
            nhead=nhead,
            use_upsample=use_upsample,
            num_hier=num_hier,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Forward pass.
        Args:
            x: Input tensor of shape (Batch, 3, Time, Height, Width) in YYY format.
        Returns:
            Saliency map tensor of shape (Batch, Height, Width).
        """
        return self.net(x)
    ...
    def from_pretrained(
        cls,
        repo_id: str = "lagosproject/paven",
        filename: str = "paven_vinet_yyy.pt",
        device: Optional[Union[str, torch.device]] = None,
        **kwargs
    ) -> "PavenModel":
        """
        Download and instantiate the PAVEN model from Hugging Face Hub.
        """
        try:
            from huggingface_hub import hf_hub_download
        except ImportError:
            raise ImportError(
                "huggingface_hub is required to use from_pretrained(). "
                "Install it via: pip install huggingface_hub"
            )

        print(f"[PAVEN] Fetching weights '{filename}' from Hugging Face Hub: '{repo_id}'...")
        weights_path = hf_hub_download(repo_id=repo_id, filename=filename)
        return cls.from_file(weights_path, device=device, **kwargs)
    ...
    def from_file(
        ...
        **kwargs
        ...
        if missing:
            print(f"[PAVEN] Note: {len(missing)} missing keys during loading.")
        if unexpected:
            print(f"[PAVEN] Note: {len(unexpected)} unexpected keys during loading.")
